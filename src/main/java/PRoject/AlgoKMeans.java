...
public class AlgoKMeans
{
    ...
    protected static ArrayList<Float> errors=new ArrayList<Float>();
	...
	public AlgoKMeans() throws IOException
	{
		...
		FileReader fichier=new FileReader("E:/java/color  hist/image.txt");
		BufferedReader bufferR=new BufferedReader(fichier);
		
		
		FileWriter fichier_w=new FileWriter("E:/java/color  hist/image.txt");
		BufferedWriter bufferW=new BufferedWriter(fichier_w);
		for(int i=0;i<1000;i++) {
			bufferW.write("E:/java/color  hist/img/"+i+".jpg");
			bufferW.newLine();
		}
		bufferW.close();
		fichier_w.close();
